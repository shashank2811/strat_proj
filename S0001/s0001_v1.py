...
# Database connection parameters for the indices_spot_ieod database
db1_params = {
    "host": "194.163.169.162",
    "database": "indices_spot_ieod",
    "user": "backtestuser",
    "password": "BaCkTeSt@2019"
}

# Database connection parameters for the fnodata2019 database
db2_params = {
    "host": "194.163.169.162",
    "database": "fnodata2019",
    "user": "backtestuser",
    "password": "BaCkTeSt@2019"
}
...
def main():
    ...
    query1 = "SELECT tr_date, tr_time, tr_close,stock_name FROM spot_indices_ieod_gdfl"
    cursor_spot.execute(query1)
    bnifty_df=pd.DataFrame(cursor_spot.fetchall(), columns=["tr_date", "tr_time", "tr_close","stock_name"])
    ...
    # Close the cursors and connections when done
    cursor_spot.close()
    conn1.close()
    cursor_fnoied.close()
    conn2.close()
...
if __name__ == "__main__":
    ...
    # Connect to the spotieod database
    conn1 = psycopg2.connect(**db1_params)
    
    # Create a cursor for the spotieod database
    cursor_spot = conn1.cursor()
   
    # Connect to the fnodata2019 database
    conn2 = psycopg2.connect(**db2_params)
    
    # Create a cursor for the fnodata2019 database
    cursor_fnoied = conn2.cursor()
    # Execute SQL query to select all columns
    table_name = "fnoieod_banknifty"  # Replace with your table name
    query = f"SELECT * FROM {table_name}"
    cursor_fnoied = conn2.cursor()
    cursor_fnoied.execute(query)

    # Create a DataFrame with all columns
    columns = [desc[0] for desc in cursor_fnoied.description]  # Extract column names
    fnoieddf = pd.DataFrame(cursor_fnoied.fetchall(), columns=columns)
    print(fnoieddf.head())
