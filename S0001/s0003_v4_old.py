...
def get_entry_price(tr_date, tr_time, otype, strike_price,fnoieddf):
    ...
    """
    print(tr_date, tr_time, otype, strike_price)
    ...
    if not filtered_data.empty:
        entry_price = filtered_data.iloc[0]["tr_close"]
    else:
        entry_price = float("NaN")  # Use NaN as a placeholder for missing data
    print(entry_price)
    return entry_price
...
def apply_exit_conditions_stop(
    tr_date,
    strike_price,
    otype,
    target,
...
def prepare_bank_nifty(bnifty_df, entry_time,fnoieddf,stock_name):
    ...
    Returns entry price
    """
# Select relevant rows based on entry_time and stock_name
    selected_data = bnifty_df[(bnifty_df["tr_time"] == entry_time) & (bnifty_df["stock_name"] == stock_name)].copy()

    # Duplicate the selected data to create rows for "CE" and "PE"
    bank_nifty = pd.concat([selected_data] * 2, ignore_index=True)

    # Assign "CE" and "PE" to alternate rows
    bank_nifty["otype"] = ["CE", "PE"] * (len(bank_nifty) // 2)

    # Round the strike price to the nearest 100
    bank_nifty["strike_price"] = (bank_nifty["tr_close"] // 100) * 100

    # Calculate entry price for CE and PE simultaneously
    bank_nifty["entry_price"] = bank_nifty.apply(
        lambda row: get_entry_price(row["tr_date"], row["tr_time"], row["otype"], row["strike_price"], fnoieddf),
        axis=1,
    )
...
def add_lotsize_column_stop(lotsize_df, fnoied_df):
    """It matches the dates from the resulting dataframe
      with the lotsize dataframe and
      takes the respective values of lotsize column
    and places it in the resulting data frame
    Returns lotsize column
    """
    # Check if fnoied_df is a DataFrame
    if not isinstance(fnoied_df, pd.DataFrame):
        raise ValueError("fnoied_df should be a DataFrame")

    # Create a dictionary mapping tr_date to Lot_Siz

    # Use the tr_date from the single-row DataFrame to add the 'lotsize' column
    if not fnoied_df.empty:
        fnoied_df["lotsize"] = 20

    return fnoied_df

def calculate_target_stop_for_stoploss(stoploss_value,target_value,fnoieddf,find_target_stoploss,new_entry_time):

    columns_to_copy = ['tr_date', 'tr_time','tr_close','strike_price','stock_name','otype']
    stoploss_df=find_target_stoploss[columns_to_copy].copy()
    stoploss_df['tr_time']=new_entry_time
    print(stoploss_df)
    print(fnoieddf['tr_date'],fnoieddf['tr_time'],fnoieddf['otype'],fnoieddf['strike_price'])
    # Calculate entry price for CE and PE simultaneously
    stoploss_df["entry_price"] = get_entry_price(stoploss_df["tr_date"],stoploss_df["tr_time"],stoploss_df["otype"],stoploss_df["strike_price"], fnoieddf)
    print(stoploss_df)
     # Convert 'stoploss_value' and 'target_value' to decimal.Decimal so that they are of high precision
    stoploss_value = decimal.Decimal(str(stoploss_value))
    target_value = decimal.Decimal(str(target_value))
    stoploss_df["entry_price"]=decimal.Decimal(str(stoploss_df["entry_price"]))
    # Create a DataFrame of boolean values indicating whether each element is int or float  

    stoploss_df["target"] = stoploss_df["entry_price"] * target_value
    stoploss_df["stoploss"]=stoploss_df["entry_price"] * stoploss_value
    # stoploss_df["target"] = stoploss_df["target"].round(3)
    # stoploss_df["stoploss"] = stoploss_df["stoploss"].round(3)
    print(stoploss_df)
    return stoploss_df
...
def process_data_for_date(bnifty_df, fnoieddf, lotsize_df):
    ...
    stoploss_rows_df = pd.DataFrame(columns=["tr_date", "tr_time", "tr_close", "strike_price", "stock_name", "otype", "entry_price", "target", "stoploss", "exit_type", "exit_price", "exit_time", "lotsize", "PNL"])
    ...
    find_target_stoploss_df = prepare_bank_nifty(bnifty_df,ENTRY_TIME,fnoieddf,STOCK_NAME)

    # Step 3: Calculate target and stoploss columns
    bank_nifty_df = calculate_target_stoploss(
        stoploss_value, target_value, find_target_stoploss_df
    )
    ...
    find_exit_conditionss = find_exit_conditions.drop(columns=columns_to_drop, errors="ignore")
    find_exit_conditionss["exit_price"] = find_exit_conditionss["exit_price"].apply(
        decimal.Decimal
    )
    # Step 6: Adding Profit and Loss Column
    find_exit_conditionss["PNL"] = (
        find_exit_conditionss["entry_price"] - find_exit_conditionss["exit_price"]
    ) * find_exit_conditionss["lotsize"]
    find_exit_conditionss[["PNL", "tr_close", "entry_price", "exit_price"]] = find_exit_conditionss[["PNL", "tr_close", "entry_price", "exit_price"]].applymap(lambda x: round(x, 3) if x is not None else None)
    print(find_exit_conditions)
    stoploss_rows = bank_nifty_df[
        (bank_nifty_df["exit_type"] == "STOPLOSS")
    ]
    if stoploss_rows.empty:
        ...
        for _, stoploss_row in stoploss_rows.iterrows():
            ...
            find_stoploss_rows = calculate_target_stop_for_stoploss(stoploss_value,target_value,fnoieddf,stoploss_row,new_entry_time)
            find_stoploss_rows["exit_type"] = None
            find_stoploss_rows["exit_price"] = None
            find_stoploss_rows["exit_time"] = None

            find_stoploss_rows[["exit_type", "exit_price", "exit_time"]] = apply_exit_conditions_stop(
                    find_stoploss_rows["tr_date"],
                    find_stoploss_rows["strike_price"],
                    find_stoploss_rows["otype"],
                    find_stoploss_rows["target"],
                    find_stoploss_rows["stoploss"],
                    SQUAREOFF_TIME,
                    new_entry_time,
                    fnoieddf
                )
            print("final op")
            print(find_stoploss_rows)
            single_row_df = pd.DataFrame([find_stoploss_rows])
            print(single_row_df)
            find_exit_conditions = add_lotsize_column_stop(lotsize_df, single_row_df)
            columns_to_drop = [
                "tr_open",
                "tr_high",
                "tr_low",
            ...
            stoploss_rows_df = stoploss_rows_df.append(find_exit_conditions, ignore_index=True)
            print(stoploss_rows_df)
    final_result_df = pd.concat([stoploss_rows_df, find_exit_conditionss], ignore_index=True)
    print("---------------------------------TOTSAKKKKKKK-----------------")
...
def main():
   ...
   # reading the csv and excel file
    ...
    s0001_v4_2019 = pd.concat(data_2019, ignore_index=True)
    print(s0001_v4_2019)

    # Output data to a single CSV file for all dates
    output_csv_path = "S0001_v4_2019.csv"
    s0001_v4_2019.to_csv(output_csv_path, index=False)
