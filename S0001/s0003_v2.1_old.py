...
def process_data_for_date(bnifty_df, fnoieddf,lotsize_df):
    ...
    # Ensure that stoploss_accumulator has the same columns as find_close_price
    stoploss_accumulator = stoploss_accumulator.reindex(columns=find_close_price.columns)
    ...
    # Use np.select to calculate 'PNL' based on exit conditions
    pnl_conditions = [
        find_exit_conditions["exit_type"] == "STOPLOSS",
        find_exit_conditions["exit_type"] == "TARGET",
        find_exit_conditions["exit_type"] == "SQOFF"
    ]
    pnl_values = [
        ((find_exit_conditions["temp_entry_price"] - find_exit_conditions["exit_price"]) * find_exit_conditions["lotsize"]),
        ((find_exit_conditions["temp_entry_price"] - find_exit_conditions["exit_price"]) * find_exit_conditions["lotsize"]),
        ((find_exit_conditions["temp_entry_price"] - find_exit_conditions["exit_price"]) * find_exit_conditions["lotsize"])
    ]

    find_exit_conditions["PNL"] = np.select(pnl_conditions, pnl_values, default=0)
...
def main():
    ...
    for date in date_range:
        ...
        tr_close, stock_name,
        strike_price, otype,week_expiry_date FROM fnoieod_nifty
        WHERE stock_name='{STOCK_NAME}' AND
        tr_date='{date.strftime('%Y-%m-%d')}'
        ...
        data_for_date.set_index("tr_date", inplace=True)
        # Append data_for_date to the list of DataFrames
        data_2019.append(data_for_date)

    # Concatenate all DataFrames into a single DataFrame
    s0002_v2_1_2019 = pd.concat(data_2019, ignore_index=True)
    print(s0002_v2_1_2019)
  
    output_csv_path = "S0002_v2.1_2019.csv"
    s0002_v2_1_2019.to_csv(output_csv_path, index=False)
