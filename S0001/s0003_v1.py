...
def get_spot_close_price(row, entry_time, bnifty_df):
    ...
    if not filtered_data.empty:
        ...
        rounded_spot_price = float("nan")  # Use NaN as a placeholder for missing data
    print("-----------------------------------ROUNDED SPOT PRICE--------------------------------")
    print(rounded_spot_price)
    return rounded_spot_price
...
def get_closest_strike_price(
 group, entry_time, closest_val,bnifty_df,trigger_val
    ...
    find_target_stoploss["strike_price"] = find_target_stoploss.apply(
...
def main():
    ...
    s0003_v1_2019 = pd.concat(data_2019, ignore_index=True)
    print(s0003_v1_2019)
  
    output_csv_path = "S0003_v1_2019.csv"
    s0003_v1_2019.to_csv(output_csv_path, index=False)
